...
export const storeVideoDetails = async (
  videoDetailsMap: Record<string, IVideoDetail[]>,
  outputDir: string,
) => {
  ...
  for (let idx = 0; idx < channelCount; idx++) {
    ...
    console.log(
      `${channelDisplayName}: fetching ${videoDetails.length} videos...`,
    );

    for (let jdx = 0; jdx < videoDetails.length; jdx++) {
      const videoDetail = videoDetails[jdx];

      const { videoFileName, videoFullpath, isStored, error } =
        await storeVideo(videoDetail, outputDir);
      ...
      if (isStored) {
        ...
        console.log(
          `${channelDisplayName}: failed ${videoDisplayName}(${videoDetail.videoId}) - ${error}`,
        );
      }
    }

    console.log(
      `${channelDisplayName}: completed ${channelSuccessCount}/${videoDetails.length} videos...`,
    );
    totalSuccessCount += channelSuccessCount;
  }

  console.log(
    `Finished. completed ${totalSuccessCount} videos for all target channels`,
  );
};
