...
const getDailyVideoDetails = async (
  { channels, fromDate, toDate }: IProps,
  logger: ILogger,
) => {
  ...
  logger(
    LogLevel.info,
    LOGGER_LOCTION,
    `Fetching video ID for ${channels.length} channel(s).`,
    channels,
  );
...
const getVideoDetailsFromVideoIds = async (
  videoIds: string[],
  logger: ILogger,
) => {
...
const getPlayListVideoDetails = async (playlistId: string, logger: ILogger) => {
  ...
  const filteredVideoDetails = await getVideoDetailsFromVideoIds(
    videoIds,
    logger,
  );
  ...
  logger(
    LogLevel.info,
    LOGGER_LOCTION,
    `Fetched ${filteredVideoDetails.length} videos for ${playlistId}`,
    {
...
(async () => {
  ...
  const { channelFilepath, outputDir, fromDate, toDate } =
    !hasDesignatedTarget && isInteractive ? await runInteractive() : rest;
